...
class TestTweepyHunterUserQuerying(TestCase):
    ...
    def test_get_no_pinned_tweet_user(self):
        single_user_test('NoPinnedTweetUser', no_pinned_tweet_user_response,
                         user_assert_func=assert_no_pinned_tweet_user)
    ...
    def test_get_users(self):
        run_stream(['NoPinnedTweetUser', 'TestUser', 'ErrorUser'], mixed_response,
                   user_assert_func=assert_users,
                   error_assert_func=assert_user_not_exist_error)
        mock_tweepy_client.get_users.assert_called_once_with(usernames=['NoPinnedTweetUser', 'TestUser', 'ErrorUser'],
                                                             **TweepyHunter.user_api_params)
...
mixed_response = tweepy.Response(data=[normal_user_response.data[0], no_pinned_tweet_user_response.data[0]],
