...
class TestUserCreatedFilterRule(TestCase):
    def test_check_time_order(self):
        assert_that(
            calling(UserCreatedFilterRule.build).with_args({"before": "2018-01-01", "after": "2020-01-01"}),
...
class IntComparingFilterRuleTestCase(TestCase):
    ...
    def test_check_number_order(self):
        assert_that(
            calling(self.build_func).with_args({"less_than": self.num(10), "more_than": self.num(20)}),
