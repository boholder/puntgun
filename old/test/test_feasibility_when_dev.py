...
class TestLibrary(TestCase):
    ...
    def test_rx_error_delivering(self):
        ...
        mix.pipe(op.filter(lambda x: isinstance(x, str))).subscribe(
            on_next=lambda x: print(x), on_error=lambda e: print("error str:", e)
        )

        mix.pipe(op.filter(lambda x: isinstance(x, int))).subscribe(
            on_next=lambda x: print(x), on_error=lambda e: print("error number:", e)
        )
    ...
    def test_observable_emit_time(self):
        ...
        rx.range(1, 4).pipe(op.map(judge_on_one_context)).subscribe(
            lambda x: x.subscribe(lambda t: print(f"==={t}==="))
        )
